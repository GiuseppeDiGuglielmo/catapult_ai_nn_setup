...
flow run /HLS4ML/gen_hls4ml $sfd/model_asic.py --reuse_factor 4

# Now 'dofile' the project
if { [file exists [pwd]/my-Catapult-test_asic4] } {
  set_working_dir [pwd]/my-Catapult-test_asic4
  dofile build_prj.tcl
  go switching
  flow run /PowerAnalysis/report_pre_pwropt_Verilog
  flow run /HLS4ML/collect_reports
} else {
  logfile message "HLS4ML failed to generate the model in directory '[pwd]/my-Catapult-test_asic4'\n" error
}
